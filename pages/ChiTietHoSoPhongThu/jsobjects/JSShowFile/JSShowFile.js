export default {
	ShowSDTC: async () => {
		const path = Table3.selectedRow.sodotuyencap;
		if (!path) {
	...
	ShowSDLG: async () => {
		const path = Table3.selectedRow.sodologic;
		if (!path) {
			showAlert("Không có đường dẫn file!", "error");
			return null;
		}
	...
	ShowSTR: async () => {
		const path = Table3.selectedRow.soturack;
		if (!path) {
			showAlert("Không có đường dẫn file!", "error");
			return null;
		}
	...
	ShowDDF: async () => {
		const path = Table3.selectedRow.duongdanfile;
		if (!path) {
			showAlert("Không có đường dẫn file!", "error");
			return null;
		}
