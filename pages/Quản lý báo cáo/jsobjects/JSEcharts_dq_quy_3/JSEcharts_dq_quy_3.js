export default {
    async fetchData() {
		...
		while (!success && retryCount < maxRetries) {
			try {
				await Countmuciteudq_quy_3.run(); // chờ hàm chạy xong
				if (Countmuciteudq_quy_3.data && Countmuciteudq_quy_3.data.length >= 0) {
					success = true;
				} else {
    ...
    // Giả sử dữ liệu đầu vào là BaoCaoDoQuetTheoQuy.data
    const dataRows = Countmuciteudq_quy_3.data.map(row => ({
      year: String(row["nam"]),
      q1: Number(row["quy1"]),
      q2: Number(row["quy2"]),
      q3: Number(row["quy3"]),
