export default {
  ShowSDTC: async () => {
    const path = view_form.formData.sodotuyencap;
    if (!path) {
	...
	ShowSDLG: async () => {
    const path = view_form.formData.sodologic;
    if (!path) {
      showAlert("Không có đường dẫn file!", "error");
      return null;
    }
	...
	ShowSTR: async () => {
    const path = view_form.formData.soturack;
    if (!path) {
      showAlert("Không có đường dẫn file!", "error");
      return null;
    }
	...
	ShowDDFKH: async () => {
		const path = TableKH.selectedRow.duongdanfile;
		if (!path) {
			showAlert("Không có đường dẫn file!", "error");
			return null;
		}
	...
	ShowDDFHSTS: async () => {
		const path = TableHSTS.selectedRow.duongdanfile;
		if (!path) {
			showAlert("Không có đường dẫn file!", "error");
			return null;
		}
	...
	ShowDDFBC: async () => {
		const path = JSObject1.getTriggeredRow().duongdan;
		if (!path) {
			showAlert("Không có đường dẫn file!", "error");
			return null;
		}
